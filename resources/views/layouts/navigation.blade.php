<nav x-data="{ open: false }" class="bg-white/70 backdrop-blur-md border-b border-white/20 sticky top-0 z-50 shadow-sm">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <!-- Settings Dropdown / Logout for Agents -->
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @if(Auth::user()->role === 'super_admin' || Auth::user()->role === 'admin')
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-4 py-2 border border-slate-200 text-sm leading-4 font-medium rounded-xl text-slate-600 bg-white/50 hover:text-slate-900 transition ease-in-out duration-150">
                ...
                @else
                    <div class="flex items-center space-x-4">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="p-2 text-slate-400 hover:text-red-600 transition-colors" title="Déconnexion">
